...
import type { RlmRunHandler, RlmRunResult } from "../rlm-runtime.ts";
...
function decode(frames: Buffer[], key: string): JupyterMessage | null {
	...
	const expected = sign(parts, key);
	const received = frames[i + 1];
...
function makeConnection(): { info: ConnectionInfo; path: string; tempDir: string } {
	...
	const tempDir = mkdtempSync(join(tmpdir(), "pi-rlm-kernel-"));
...
export class KernelManager {
	...
	private startShellPump(): void {
		...
		this.shellPumpPromise = (async () => {
			try {
				for await (const frames of shell) {
					// Drain signed execute replies. Execution output and completion arrive on IOPub.
					decode(frames, connection.key);
				}
			} catch (error) {
				if ((this.state as string) !== "shutdown") {
					console.error(`[kernel] shell pump failed: ${errorMessage(error)}`);
	...
	private async runIopubPump(): Promise<void> {
		...
		try {
			for await (const frames of iopub) {
				const incoming = decode(frames, connection.key);

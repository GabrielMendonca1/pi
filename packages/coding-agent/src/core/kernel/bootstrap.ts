...
const RUNTIME_REQUIREMENT = "pi-rlm-runtime";
...
export function getKernelVenvDir(): string {
	...
	return path.join(os.homedir(), ".pi", "agent", "kernel-venv");
}
...
function runtimeCandidateDirs(): string[] {
	...
	return [
		path.resolve(moduleDir, "..", "..", "pi-rlm-runtime"),
		path.resolve(moduleDir, "..", "..", "..", "..", "..", "pi-rlm-runtime"),
	];
}
...
function formatBootstrapFailure(error: unknown): Error {
	return new Error(
		`Failed to set up the Python kernel runtime. ${errorMessage(error)}\n` +
			...
			"Set PI_RLM_KERNEL_PYTHON to a Python with ipykernel and a current pi-rlm-runtime installed to skip auto-bootstrap.",
	);
}
...
async function ensureKernelPythonUncached(): Promise<string> {
	...
	if (override) {
		...
		if (!(await hasPiRlmRuntime(python))) missing.push("a current pi-rlm-runtime with callable rlm.run");
		if (missing.length === 0) return python;
		throw new Error(`PI_RLM_KERNEL_PYTHON points to a Python missing ${missing.join(" and ")}: ${python}`);
	}

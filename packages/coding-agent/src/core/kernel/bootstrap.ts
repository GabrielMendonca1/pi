...
const RUNTIME_REQUIREMENT = "pi-rlm-runtime@1";
...
function ensureKernelPythonKey(): string {
	return [
		process.env.PI_RLM_KERNEL_PYTHON ?? "",
		process.env.PI_RLM_KERNEL_VENV ?? "",
		process.env.HOME ?? "",
...
export function getKernelVenvDir(): string {
	const override = process.env.PI_RLM_KERNEL_VENV;
...
function getXdgKernelVenvDir(): string {
	...
	return path.join(dataHome, "pi", "agent", "kernel-venv");
}
...
async function resolveWritableKernelVenvDir(): Promise<string> {
	...
	try {
		...
		if (process.env.PI_RLM_KERNEL_VENV) {
		...
		try {
			...
			throw new Error(
				`couldn't create kernel venv directory at ${primary} or ${fallback}; set PI_RLM_KERNEL_PYTHON to a python with ipykernel installed. ${errorMessage(fallbackError)}`,
			);
		}
	}
}
...
async function hasPiRlmRuntime(python: string): Promise<boolean> {
...
async function ensureUv(): Promise<string> {
	...
	try {
		...
		throw new Error(
			`couldn't install uv from astral.sh; install it yourself: curl -LsSf https://astral.sh/uv/install.sh | sh, then re-run pi. ${errorMessage(error)}`,
		);
	}
...
function runtimeCandidateDirs(): string[] {
	...
	return [path.resolve(moduleDir, "..", "..", "runtime"), path.resolve(moduleDir, "..", "..", "..", "runtime")];
}
...
async function resolveRuntimeRequirement(): Promise<string> {
	...
	throw new Error("bundled pi RLM runtime not found");
}
...
async function bootstrapVenv(venv: string): Promise<void> {
	...
	const stagingDir = await mkdtemp(path.join(os.tmpdir(), "pi-rlm-runtime-"));
	const stagedRuntime = path.join(stagingDir, "runtime");

	try {
		await cp(runtimeRequirement, stagedRuntime, { recursive: true });
		await run(uv, ["python", "install", PYTHON_VERSION]);
		await run(uv, ["venv", venv, "--python", PYTHON_VERSION, "--seed"]);
		await run(uv, ["pip", "install", "--python", python, IPYKERNEL_REQUIREMENT, stagedRuntime]);
...
function formatBootstrapFailure(error: unknown): Error {
	return new Error(
		`Failed to set up the Python kernel runtime. ${errorMessage(error)}\n` +
			"First-time setup needs internet to install uv, Python, ipykernel, and pi-rlm-runtime; once set up, pi runs offline. " +
			"Set PI_RLM_KERNEL_PYTHON to a Python with ipykernel installed to skip auto-bootstrap.",
	);
}

async function ensureKernelPythonUncached(): Promise<string> {
	const override = process.env.PI_RLM_KERNEL_PYTHON;
	if (override) {
		const python = path.resolve(expandHome(override));
		if (await hasIpykernel(python)) return python;
		throw new Error(`PI_RLM_KERNEL_PYTHON points to a Python that cannot import ipykernel: ${python}`);
	}

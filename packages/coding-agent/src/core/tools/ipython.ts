...
import { KernelManager } from "../kernel/index.ts";
import type { RlmRunHandler } from "../rlm-runtime.ts";
import { wrapToolDefinition } from "./tool-definition-wrapper.ts";

const RLM_BOOTSTRAP_CODE = `
try:
    import rlm as _pi_rlm_module
    rlm = _pi_rlm_module.rlm
except Exception as _pi_rlm_error:
    _PI_RLM_IMPORT_ERROR = str(_pi_rlm_error)

    class _PiMissingRlm:
        async def run(self, prompt, **kwargs):
            raise RuntimeError(
                "pi-rlm-runtime is not installed in this IPython kernel. "
                "Remove ~/.pi/agent/kernel-venv so pi can rebuild it, or set "
                "PI_RLM_KERNEL_PYTHON to a kernel environment with pi-rlm-runtime installed. "
                f"Import error: {_PI_RLM_IMPORT_ERROR}"
            )
...
export function createIpythonToolDefinition(
	...
	return {
		...
		promptGuidelines: [
			"Use IPython as a persistent control environment; project commands should run through the project's own environment",
			`A callable rlm is preloaded in IPython; use await rlm("sub-task") for recursive child sessions when delegation helps`,
		],
